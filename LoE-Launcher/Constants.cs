namespace LoE_Launcher;

public static class Constants
{
    public const string GameName = "LoE";
    public const string GameFullName = "Legends of Equestria";

    public const string CacheDirectoryName = "Cache";
    public const string BackgroundImageFileName = "Background.png";
    public const string ChangelogFileName = "Changelog.txt";

    public const string BackgroundImageUrl = "https://loedata.legendsofequestria.com/data/Background.png";
    public const string ChangelogUrl = "https://loedata.legendsofequestria.com/data/Changelog.txt";

    public const string YouTubeUrl = "https://www.youtube.com/@legendsofequestria";
    public const string DiscordUrl = "https://discord.com/invite/legendsofeq";
    public const string XUrl = "https://x.com/LegendsofEq";
    public const string FacebookUrl = "https://www.facebook.com/LegendsOfEquestria";
}
